<x-app-layout>
    ...
    <div style="padding: 20px;background-color:white">
        ...
        @endif
        @if (Cart::instance(Auth::user()->id)->count())
            <form method="get" action="{{ route('cart.clear',Auth::user()->id) }}" style="margin-bottom: 1rem">
                @csrf
            ...
            <tbody>
                ...
                @foreach (Cart::instance(Auth::user()->id)->content() as $cartItem)
                    ...
                    <tr data-id="{{ $cartItem->id }}">
                        ...
                        <td data-th="Quantity">
                            <form method="post" action="{{url('/updatecart/'.Auth::user()->id.'/'. $cartItem->rowId) }}">
                                @method('put')
                                @csrf
                                <span>
            ...
            <tfoot>
                <tr>
                    <td colspan="6" class="text-right">
                        ...
                        <h5><strong>{{__('Total Price')}} ${{ Cart::instance(Auth::user()->id)->priceTotal() }}</strong></h5>

                        <div>
                            <h5><b>Tax : {{ Cart::instance(Auth::user()->id)->tax() }} $</b></h5>

                        </div>
                        @if (session()->get('used'))
                            @if (Cart::instance(Auth::user()->id)->priceTotal('2', '.', '') > 10000)
                                @if (Cart::instance(Auth::user()->id)->GetPercentDiscount()) 
                                    <hr>
                                    <div style="color:green">

                                        <h5><b>{{__('New Total')}} : {{ Cart::instance(Auth::user()->id)->total() }} $</b></h5>
                                        <div> {{__('Discount')}} : {{ Cart::instance(Auth::user()->id)->discount() }}</div>
                                        <div> {{__('Percentage')}} : {{ Cart::instance(Auth::user()->id)->GetPercentDiscount() }}%
                                        </div>
                            ...
                            @else
                                @if (Cart::instance(Auth::user()->id)->GetFixedDiscount())
                                    <hr>
                                    <div style="color:green">
                                        <h5><b>{{__('New Total')}} : {{ Cart::instance(Auth::user()->id)->GetTotalFixed() }}
                                                $</b></h5>
                                    {{__('Discount')}} :   {{ Cart::instance(Auth::user()->id)->GetFixedDiscount() }}$
                                    </div>
                                @endif
                            @endif
                        @else
                            @if (Cart::instance(Auth::user()->id)->count())
                                <h5><strong>Total ${{ Cart::instance(Auth::user()->id)->total() }}</strong></h5>

                                <hr>
                                @if (Cart::instance(Auth::user()->id)->total('2', '.', '') < 10000)
